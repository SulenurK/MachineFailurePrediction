...
def explain_model(model, X_train_transformed, X_test_transformed, sample_index=0, top_n_features=10, save_path=None, model_type='tree'):
    """
    ...
    # Plot 1: Feature Importance (Bar Summary)
    shap.summary_plot(shap_values, X_test_transformed, plot_type="bar", max_display=top_n_features, show=False)
    plt.title("Feature Importance (SHAP)")
    if save_path:
        plt.savefig(f"{save_path}/shap_summary_bar.png", bbox_inches='tight', dpi=300)
    plt.show()

    # Plot 2: SHAP Value Distribution (Dot Summary)
    shap.summary_plot(shap_values, X_test_transformed, max_display=top_n_features, show=False)
    ...
    # Plot 5: SHAP Heatmap (for small datasets only)
    try:
        if X_test_transformed.shape[0] <= 100:  # avoid overload on large data
    ...
    # Plot 6: Dependence Plot for Top Feature
    try:
        shap_values_abs = np.abs(shap_values.values).mean(axis=0)
        top_feature_idx = np.argsort(shap_values_abs)[-1]
        top_feature_name = X_test_transformed.columns[top_feature_idx]
